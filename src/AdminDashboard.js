...
const AdminDashboard = () => {
  ...
  return (
    <div style={{ height: '100vh', fontFamily: 'Calibri, sans-serif', display: 'flex', flexDirection: 'column' }}>
      <div style={{ backgroundColor: '#D6C271', padding: '20px 30px', display: 'flex', justifyContent: 'space-between', fontSize: '1.2rem', fontWeight: 'bold' }}>
      ...
      <div style={{ display: 'flex', flex: 1 }}>
        ...
        <div style={{ flex: 1, padding: '30px', backgroundColor: 'black', color: 'white' }}>
    ...
    <div>
      ...
      {properties.map((p, i) => (
        <div key={i} style={{ display: 'flex', padding: '12px 0', borderBottom: '1px solid #999', color: 'white' }}>
          ...
          <div style={{ flex: 1, color: 'red', fontWeight: 'bold', cursor: 'pointer' }} onClick={() => removeProperty(i)}>⛔</div>
        </div>
      ))}
          ...
          <div style={{
            ...
            <div style={{ marginBottom: '10px' }}>
              ...
              <select value={property} onChange={e => setProperty(e.target.value)} style={{ width: '100%' }} disabled={!location}>
                ...
                {location && propertyOptions[location].map(p => (
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px' }}>
              ...
              <button onClick={addProperty} style={{ padding: '6px 12px', backgroundColor: '#F7D774', color: '#000', border: 'none' }}>Add</button>
            </div>
          </div>
        </div>
      )}
...
const IoTDevicesStatus = () => {
  const [showModal, setShowModal] = useState(false);
  const devices = [
    { id: '001', name: 'AC Controller', type: 'HVAC', property: 'mySTAY Hotel BGC North', location: 'Living Room', status: 'Online', battery: '85%', temp: '22 °C', humidity: '55%', alerts: 'None' },
    { id: '002', name: 'Smart Lock', type: 'Security', property: 'mySTAY Hotel BGC North', location: 'Main door', status: 'Online', battery: '85%', temp: '22 °C', humidity: '55%', alerts: 'None' },
    { id: '003', name: 'Water Meter', type: 'Utility', property: 'mySTAY Hotel BGC North', location: 'Basement', status: 'Online', battery: '85%', temp: '22 °C', humidity: '55%', alerts: 'None' },
    { id: '004', name: 'Gas Sensor', type: 'Safety', property: 'mySTAY Hotel BGC North', location: 'Kitchen', status: 'Online', battery: '85%', temp: '22 °C', humidity: '55%', alerts: 'None' }
  ];
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', fontWeight: 'bold', borderBottom: '1px solid #D6C271', padding: '10px 0', color: '#D6C271', fontSize: '0.9rem' }}>
    ...
    <div>
      ...
      {alerts.map((alert, i) => (
        <div key={i} style={{
          ...
          <div style={{ flex: 1, color: alert.statusColor }}>{alert.statusIcon} {alert.status}</div>
        </div>
      ))}
    </div>
  );
    ...
    <div style={{ textAlign: 'center' }}>
      {selectedLocation ? (
        <PropertiesView
          ...
          properties={locationProperties[selectedLocation]}
        />
      ) : (
        <>
...
const MaintenanceCard = ({ title, priority, status, onClick }) => {
  return (
    ...
    >
      ...
      <h3 style={{ color: '#D6C271', 
                   fontSize: '1.2rem', 
                   marginBottom: '5px' }}>{title}
      </h3>
      <p style={{ color: 'rgba(214, 194, 113, 0.5)', 
                  fontSize: '1rem', 
                  marginBottom: '10px' }}>{priority}
      </p>
  ...
  return (
    ...
    >
      <h1 style={{ fontSize: '2rem',
                   marginBottom: '5px' }}>Financial Overview
      </h1>
      <p style={{ fontSize: '1rem', 
                  color: 'rgba(214, 194, 113, 0.7)' }}>Track rent collection and expenses
      </p>
